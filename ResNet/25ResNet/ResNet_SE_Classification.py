...
def main():
    ...
    # 使用ResNet50+SE模型
    model = ModifiedResNet50(SEBottleneck, [3, 4, 6, 3], num_classes=num_classes)
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1 + min_delta:
            ...
            torch.save(model.state_dict(), 'best_model_ResNet_SE_classification.pth')
    ...
    # 测试模型
    model.load_state_dict(torch.load('best_model_ResNet_SE_classification.pth'))
    model.eval()
    test_loss = 0.0
    test_labels = []
    test_preds = []
    ...
    # 保存DataFrame到CSV文件
    df.to_csv('test_metrics_ResNet_SE_classification.csv', index=False)
    print("Metrics saved to 'test_metrics_ResNet_SE_classification.csv'.")
