...
class ModifiedResNet50(models.ResNet):
    def __init__(self, *args, num_classes=3, **kwargs):
        super(ModifiedResNet50, self).__init__(*args, **kwargs)
        self.fc = nn.Sequential(
            nn.Dropout(0.5),  # Dropout层
            nn.Linear(self.fc.in_features, num_classes)  # 修改最后一层用于分类任务
        )
...
def main():
    ...
    # 使用ResNet50模型
    model = ModifiedResNet50(models.resnet.Bottleneck, [3, 4, 6, 3], num_classes=num_classes)
    # 修复deprecated警告：使用weights参数替代pretrained参数
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1 + min_delta:
            ...
            torch.save(model.state_dict(), 'best_model909-ResNet-classification.pth')
    ...
    # 测试模型
    model.load_state_dict(torch.load('best_model909-ResNet-classification.pth'))
    model.eval()
    test_loss = 0.0
    test_labels = []
    test_preds = []
    ...
    # 保存DataFrame到CSV文件
    df.to_csv('test_metrics909-ResNet-classification.csv', index=False)
    print("Metrics saved to 'test_metrics909-ResNet-classification.csv'.")
