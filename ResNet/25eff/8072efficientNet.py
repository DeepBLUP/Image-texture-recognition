...
class ModifiedEfficientNet(nn.Module):
    def __init__(self, num_classes=1):
        ...
        self.efficientnet = models.efficientnet_b0(pretrained=True)  # 使用 EfficientNet-B0

        # 冻结卷积层的参数，防止在训练过程中更新（可选）
        for param in self.efficientnet.features.parameters():
            param.requires_grad = False

        # 获取原始 EfficientNet 分类器的输入特征数
        num_features = self.efficientnet.classifier[-1].in_features

        # 修改分类器以适用于回归任务
        self.efficientnet.classifier = nn.Sequential(
            nn.Dropout(0.3),  # 根据需要调整 Dropout 值
            nn.Linear(num_features, num_classes)  # 修改最后一层为一个输出单元，用于回归任务
        )
...
def main():
    ...
    # 使用DenseNet模型
    model = ModifiedEfficientNet(num_classes=1)
    ...
    for epoch in range(num_epochs):
        ...
        if val_pcc > best_val_pcc:
            ...
            torch.save(model.state_dict(), r'/home/chuannong1/ZLY/2024/IMF/大理石纹等级/eff/best_model906-effNet.pth')
    ...
    # 测试模型
    model.load_state_dict(torch.load(r'/home/chuannong1/ZLY/2024/IMF/大理石纹等级/eff/best_model906-effNet.pth'))
    model.eval()
    test_loss = 0.0
    test_labels = []
    test_preds = []
    ...
    # 保存DataFrame到CSV文件
    df.to_csv('/home/chuannong1/ZLY/2024/IMF/大理石纹等级/eff/test_metrics-eff.csv', index=False)
