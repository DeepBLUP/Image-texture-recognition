...
class ModifiedVGG(nn.Module):
    def __init__(self, num_classes):
        ...
        self.selayers = nn.ModuleList([
            SELayer(64), SELayer(128), SELayer(256), SELayer(512), SELayer(512)
        ])

    def forward(self, x):
        # 应用特征提取器和SELayer
        sections = [4, 9, 16, 23, 30]  # VGG各卷积块的结束层索引
        start = 0
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_f1:
            best_f1 = val_f1
            torch.save(model.state_dict(), 'best_model_vgg_se_classification.pth')
            patience_counter = 0
    ...
    # 测试
    print("开始测试...")
    model.load_state_dict(torch.load('best_model_vgg_se_classification.pth', map_location=device))
    ...
    test_df.to_csv('test_metrics_VGG_SE_classification.csv', index=False)
    print("测试指标已保存到 test_metrics_VGG_SE_classification.csv")
    
    # 保存混淆矩阵
    cm_df = pd.DataFrame(cm)
    cm_df.to_csv('confusion_matrix_VGG_SE_classification.csv', index=False)
    print("混淆矩阵已保存到 confusion_matrix_VGG_SE_classification.csv")
