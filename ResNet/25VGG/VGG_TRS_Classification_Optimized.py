...
class PorkFatDataset(Dataset):
    def __init__(self, csv_file, img_dir, transform=None, scaler=None):
...
csv_file = '/home/chuannong1/ZLY/2024/IMF/大理石纹等级/labels906.csv'
...
class Transformer(nn.Module):
    def __init__(self, d_model=256, nhead=8, num_layers=3, dim_feedforward=512):
...
# 自定义VGG+Transformer模型
class ModifiedVGG(nn.Module):
    def __init__(self, num_classes):
        super(ModifiedVGG, self).__init__()
        # 使用weights参数替代deprecated的pretrained
        ...
        # 修改分类器以适用于Transformer输入
        self.vgg.classifier = nn.Sequential(
            nn.Linear(25088, 4096),
            nn.ReLU(True),
            nn.Dropout(),
            nn.Linear(4096, 256)  # 输出256维特征用于Transformer
        )

        # Transformer 模块
        self.transformer = Transformer(d_model=256, nhead=8, num_layers=3, dim_feedforward=512)
    ...
    def forward(self, x):
        ...
        x = torch.flatten(x, 1)
        features = self.vgg.classifier(x)  # (batch_size, 256)
        
        # Transformer处理
        transformer_out = self.transformer(features)  # (batch_size, 256)
...
def main():
    ...
    # 加载数据集（不进行归一化）
    dataset = PorkFatDataset(csv_file=csv_file, img_dir=img_dir, transform=train_transform, scaler=None)
    ...
    # 初始化模型
    model = ModifiedVGG(num_classes=num_classes).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_f1:
            best_f1 = val_f1
            torch.save(model.state_dict(), 'best_model_vgg_trs_classification.pth')
            patience_counter = 0
    ...
    # 测试
    print("开始测试...")
    model.load_state_dict(torch.load('best_model_vgg_trs_classification.pth', map_location=device))
    ...
    test_df.to_csv('test_metrics_VGG_TRS_classification.csv', index=False)
    print("测试指标已保存到 test_metrics_VGG_TRS_classification.csv")
    
    # 保存混淆矩阵
    cm_df = pd.DataFrame(cm)
    cm_df.to_csv('confusion_matrix_VGG_TRS_classification.csv', index=False)
    print("混淆矩阵已保存到 confusion_matrix_VGG_TRS_classification.csv")
