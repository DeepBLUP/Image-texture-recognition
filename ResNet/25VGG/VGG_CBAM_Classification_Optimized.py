...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_f1:
            best_f1 = val_f1
            torch.save(model.state_dict(), 'best_model_vgg_cbam_classification.pth')
            patience_counter = 0
    ...
    # 测试
    print("开始测试...")
    model.load_state_dict(torch.load('best_model_vgg_cbam_classification.pth', map_location=device))
    ...
    test_df.to_csv('test_metrics_VGG_CBAM_classification.csv', index=False)
    print("测试指标已保存到 test_metrics_VGG_CBAM_classification.csv")
    
    # 保存混淆矩阵
    cm_df = pd.DataFrame(cm)
    cm_df.to_csv('confusion_matrix_VGG_CBAM_classification.csv', index=False)
    print("混淆矩阵已保存到 confusion_matrix_VGG_CBAM_classification.csv")
