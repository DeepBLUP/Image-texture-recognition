    ...
    def __init__(self, num_classes=3):
        ...
        self.vgg.classifier = nn.Sequential(
            *list(self.vgg.classifier.children())[:-1],  # 保留除了最后一层之外的所有层
            ...
            nn.Linear(num_features, num_classes)  # 修改最后一层为分类任务
        )
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1 + min_delta:
            ...
            torch.save(model.state_dict(), 'best_model_VGG_classification.pth')
    ...
    # 测试模型
    model.load_state_dict(torch.load('best_model_VGG_classification.pth'))
    model.eval()
    test_loss = 0.0
    test_labels = []
    test_preds = []
    ...
    # 保存DataFrame到CSV文件
    df.to_csv('test_metrics_VGG_classification.csv', index=False)
    print("Metrics saved to 'test_metrics_VGG_classification.csv'.")
