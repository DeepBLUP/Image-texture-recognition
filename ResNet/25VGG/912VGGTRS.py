...
class ModifiedVGG(nn.Module):
    def __init__(self):
        ...
        self.vgg.classifier = nn.Sequential(
            *list(self.vgg.classifier.children())[:-1],  # 保留除了最后一层之外的所有层
            nn.Dropout(0.5),  # 添加 Dropout 层以减少过拟合
            nn.Linear(num_features, 256)  # 修改最后一层为一个中间层，用于 Transformer 输入
        )

        # Transformer 模块
        self.transformer = nn.Transformer(
            d_model=256,  # Transformer 输入的特征维度
            nhead=8,  # 多头注意力机制的头数
            num_encoder_layers=3,  # Transformer 编码器层数
            num_decoder_layers=3,  # Transformer 解码器层数
            dim_feedforward=512  # 前馈层的维度
        )
        self.fc_out = nn.Linear(256, 1)  # 最终回归层

    def forward(self, x):
        # VGG 网络的前向传播
        features = self.vgg(x)
        features = features.unsqueeze(0)  # 增加序列维度，Transformer 需要
        transformer_out = self.transformer(features, features)
        transformer_out = transformer_out.mean(dim=0)  # 处理 Transformer 输出
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if val_pcc > best_val_pcc:
            ...
            torch.save(model.state_dict(), r'/home/chuannong1/ZLY/2024/IMF/大理石纹等级/VGG/best_model906-VGGTRS.pth')
    ...
    # 测试模型
    model.load_state_dict(torch.load(r'/home/chuannong1/ZLY/2024/IMF/大理石纹等级/VGG/best_model906-VGGTRS.pth'))
    model.eval()
    test_loss = 0.0
    test_labels = []
    test_preds = []
    ...
    # 保存DataFrame到CSV文件
    df.to_csv('/home/chuannong1/ZLY/2024/IMF/大理石纹等级/VGG/test_metrics-VGGTRS.csv', index=False)
