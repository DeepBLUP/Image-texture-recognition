...
# Custom ResNet50 model with Transformer
class ModifiedResNet50(nn.Module):
    def __init__(self, transformer_input_dim, transformer_hidden_dim, transformer_output_dim):
        ...
        # Transformer
        self.transformer = Transformer(transformer_input_dim, transformer_hidden_dim, transformer_output_dim)
        
        # Final FC layer for regression
        self.fc = nn.Linear(num_ftrs + transformer_output_dim, 1)
    ...
    def forward(self, x):
        ...
        transformed_features = self.transformer(features.unsqueeze(1))
        
        # Concatenate ResNet features and Transformer output
        combined_features = torch.cat((features, transformed_features.squeeze(1)), dim=1)  # Adjusted here
...
def main():
    ...
    # 使用ResNet50模型
    model = ModifiedResNet50(transformer_input_dim=2048, transformer_hidden_dim=256, transformer_output_dim=128)
    ...
    for epoch in range(num_epochs):
        ...
        if val_pcc > best_val_pcc:
            ...
            torch.save(model.state_dict(), '/home/chuannong1/ZLY/2024/IMF/大理石纹等级/25.6/ResNet/25.6.6/best_model906-TRS.pth')
    ...
    # 测试模型
    model.load_state_dict(torch.load('/home/chuannong1/ZLY/2024/IMF/大理石纹等级/25.6/ResNet/25.6.6/best_model906-TRS.pth'))
    model.eval()
    test_loss = 0.0
    test_labels = []
    test_preds = []
    ...
    # 保存DataFrame到CSV文件
    df.to_csv('/home/chuannong1/ZLY/2024/IMF/大理石纹等级/25.6/ResNet/25.6.6/test_metrics906TRS.csv', index=False)
