...
# 自定义DenseNet模型
class ModifiedDenseNet(models.DenseNet):
    def __init__(self, *args, **kwargs):
        super(ModifiedDenseNet, self).__init__(*args, **kwargs)
        num_features = self.classifier.in_features  # 获取原始 DenseNet 分类器的输入特征数
        # 修改分类器以适用于回归任务
        self.classifier = nn.Sequential(
            nn.Dropout(0.5),  # 添加 Dropout 层以减少过拟合
            nn.Linear(num_features, 1)  # 修改最后一层为一个输出单元，用于回归任务
        )
...
def main():
    ...
    # Use ModifiedDenseNet model
    model = ModifiedDenseNet(growth_rate=32, block_config=(6, 12, 24, 16))  # Adjust parameters as needed
    # Load pretrained weights (if needed), or train from scratch
    # Example of loading pretrained DenseNet weights:
    # model.load_state_dict(models.densenet121(pretrained=True).state_dict(), strict=False)
    ...
    for epoch in range(num_epochs):
        ...
        if val_pcc > best_val_pcc:
            ...
            torch.save(model.state_dict(), r'/home/chuannong1/ZLY/2024/IMF/大理石纹等级/deseNet/best_model906-deseNet.pth')
    ...
    # 测试模型
    model.load_state_dict(torch.load(r'/home/chuannong1/ZLY/2024/IMF/大理石纹等级/deseNet/best_model906-deseNet.pth'))
    model.eval()
    test_loss = 0.0
    test_labels = []
    test_preds = []
    ...
    # 保存DataFrame到CSV文件
    df.to_csv('/home/chuannong1/ZLY/2024/IMF/大理石纹等级/deseNet/test_metrics-deseNet.csv', index=False)
